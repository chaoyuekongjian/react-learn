...
源码分析：
1. runSaga：一个函数，用于启动一个任务，一个任务的本质是generator函数，runSaga在内部得到该函数的generator，并控制生成器的每一次迭代
2. 
...
# 
- react: 组件化的UI界面处理方案
- react-router：根据地址路由，最终渲染不同组件
- redux：处理数据以及数据变化的方案(主要用于处理共享数据)
- react-redux：

- 如果一个组件，仅用于渲染一个UI界面，而没有状态(通常是一个函数组件)，该组件叫做**函数组件**
- 如果一个组件，仅用于提供数据，没有任何属于自己的UI界面，该组件叫做**容器组件**